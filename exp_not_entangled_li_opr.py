...
# experiments for LI_NONORTHO training samples
...
def training_data():
    ...
    X = np.array(deorthogonalize(X))
    Y = torch.from_numpy(X).to(torch.complex128)
    Y = Y.reshape((Y.shape[0], int(Y.shape[1] / U.shape[0]), U.shape[0])).permute(0, 2, 1)
    return Y

experiment.train_networks(U, "experiment_not_entangled/li_opr", training_data, num_tries, qnn, num_processes)
